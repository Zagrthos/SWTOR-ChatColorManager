...
namespace ChatManager.Services
{
    internal static class Updater
    {
        ...
        internal static async Task CheckForUpdateInterval()
        {
            ...
            {
                Logging.Write(LogEventEnum.Error, ProgramClass.Updater, "updateInterval Setting not set!");
                ShowMessageBox.ShowBug();
            }

            Logging.Write(LogEventEnum.Variable, ProgramClass.Updater, $"updateInterval set to: {updateInterval}");
            Logging.Write(LogEventEnum.Variable, ProgramClass.Updater, $"lastCheck set to: {lastCheck}");
            Logging.Write(LogEventEnum.Variable, ProgramClass.Updater, $"today set to: {today}");
            Logging.Write(LogEventEnum.Variable, ProgramClass.Updater, $"difference set to: {difference}");
            Logging.Write(LogEventEnum.Variable, ProgramClass.Updater, $"updateSearch set to: {updateSearch}");
        ...
        {
            Logging.Write(LogEventEnum.Method, ProgramClass.Updater, "CheckForUpdates entered");

            HttpClient client = new();
            Logging.Write(LogEventEnum.Info, ProgramClass.Updater, "HttpClient created");

            try
            {
                Logging.Write(LogEventEnum.Info, ProgramClass.Updater, "Check for Updates initiated");

                onlineVersion = new(await client.GetStringAsync(updateCheckURL));

                Logging.Write(LogEventEnum.Variable, ProgramClass.Updater, $"onlineVersion is: {onlineVersion}");

                if (onlineVersion > currentVersion)
                {
                    Logging.Write(LogEventEnum.Info, ProgramClass.Updater, "Update is available!");

                    if (ShowMessageBox.ShowUpdate(onlineVersion.ToString()))
                    {
                        if (GetSetSettings.GetUpdateDownload)
                        {
                            Logging.Write(LogEventEnum.Info, ProgramClass.Updater, "Manual download initiated");
                            OpenWindows.OpenLinksInBrowser($"{updateURL}/tag/v{onlineVersion}/");
                        }
                        else
                        {
                            Logging.Write(LogEventEnum.Info, ProgramClass.Updater, "Background download initiated");
                            await DownloadUpdate();
                        }

                        Logging.Write(LogEventEnum.Info, ProgramClass.Updater, "HttpClient disposed!");
                        client.Dispose();
                    }
                }
                else
                {
                    Logging.Write(LogEventEnum.Info, ProgramClass.Updater, "No Update available!");
                ...
                if (GetSetSettings.GetUpdateInterval != UpdateEnum.OnStartup.ToString() && !fromUser)
                {
                    ...
                    Logging.Write(LogEventEnum.Variable, ProgramClass.Updater, $"Last Update Check: {DateTime.Today}");
                }
            }
            catch (HttpRequestException ex)
            {
                Logging.Write(LogEventEnum.Error, ProgramClass.Updater, "Check for Updates failed!");
                Logging.Write(LogEventEnum.ExMessage, ProgramClass.Updater, $"{ex.Message}");
        ...
        {
            Logging.Write(LogEventEnum.Method, ProgramClass.Updater, "DownloadUpdate entered");

            updateName += $"v{onlineVersion}.exe";
            Logging.Write(LogEventEnum.Variable, ProgramClass.Updater, $"updateName set to: {updateName}");

            updateURL += $"download/v{onlineVersion}/{updateName}";
            Logging.Write(LogEventEnum.Variable, ProgramClass.Updater, $"updateURL set to: {updateURL}");
            ...
            try
            {
                Logging.Write(LogEventEnum.Info, ProgramClass.Updater, "Connection Test initiated");

                HttpResponseMessage responseMessage = await client.GetAsync(updateURL, HttpCompletionOption.ResponseHeadersRead);

                if (!responseMessage.IsSuccessStatusCode)
                {
                    Logging.Write(LogEventEnum.Error, ProgramClass.Updater, "Connection Test failed!");
                    Logging.Write(LogEventEnum.ExMessage, ProgramClass.Updater, $"{responseMessage}");
                    return;
                }

                updatePath = Path.Combine(Path.GetTempPath(), updateName);
                Logging.Write(LogEventEnum.Variable, ProgramClass.Updater, $"Download Path: {updatePath}");
                    ...
                    while ((bytesRead = await stream.ReadAsync(buffer)) != 0)
                    {
                        ...
                        if (percent > -1)
                        {
                            ...
                            if (Math.Abs(percent - lastLoggedPercent) >= 1)
                            {
                                ...
                                Logging.Write(LogEventEnum.Variable, ProgramClass.Updater, $"progress: {percent}%");
                ...
                Logging.Write(LogEventEnum.Variable, ProgramClass.Updater, $"Update downloaded to: {updatePath}");
            ...
            {
                Logging.Write(LogEventEnum.Error, ProgramClass.Updater, "Update download failed!");
                Logging.Write(LogEventEnum.ExMessage, ProgramClass.Updater, $"{ex.Message}");
            }
        }
        ...
        {
            Logging.Write(LogEventEnum.Method, ProgramClass.Updater, "InstallUpdate entered");
